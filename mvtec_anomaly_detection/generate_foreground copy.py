...
if __name__ == "__main__":
    paths = os.listdir("./mvtec_anomaly_detection")
    dirs = list()
    os.chdir("./mvtec_anomaly_detection")
    for i in range(len(paths)):
        if os.path.isdir(paths[i]):
            dirs.append(paths[i])
    to_be_processed = ["bottle", "capsule", "metal_nut", "pill", "screw", "toothbrush", "zipper", "transistor"]
    for i, item in enumerate(to_be_processed):
        if item is not "metal_nut":
            os.chdir(item)
            os.makedirs("./train/good/fore_mask", exist_ok=True)
            file_paths = glob.glob("./train/good/*.png")
            for j, file_path in enumerate(file_paths):
                ...
                print("[INFO] applying GrabCut took {:.5f} seconds".format(end - start))
            os.chdir("../")
        else:
            os.chdir(item)
            os.makedirs("./train/good/fore_mask", exist_ok=True)
