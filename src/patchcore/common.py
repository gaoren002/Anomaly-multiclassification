...
class MyNetMLP(nn.Module): 

    def __init__(self, class_number=4):
        super(MyNetMLP, self).__init__()
        self.linear1 = nn.Linear(784*1024, 128)
        self.bn = nn.BatchNorm1d(128)
        self.relu = nn.ReLU()
        self.linear2 = nn.Linear(128, class_number)
...
class MyNetmetric(nn.Module):  # deprecated

    def __init__(self):
        super(MyNetmetric, self).__init__()
...
class RelationNet(nn.Module): 
    ...
    def __init__(self, class_number=4):
        ...
        self.conv1 = nn.Conv2d(1024*2,256*2,1)
        self.bn1 = nn.BatchNorm2d(256*2)  
        self.relu1 = nn.ReLU()
        self.conv2 = nn.Conv2d(256*2,128,1)
        self.bn2 = nn.BatchNorm2d(128)
        self.relu2 = nn.ReLU()

        # self.linear1 = nn.Linear(784*512, 128)
        # self.bn3 = nn.BatchNorm1d(128)
        # self.relu3 = nn.ReLU()
        # self.linear2 = nn.Linear(128, 1)
        # self.sigmoid = nn.Sigmoid()

        self.linear11 = nn.Linear(784*128, 128)
        self.bn4 = nn.BatchNorm1d(128)
        self.relu4 = nn.ReLU()
        self.linear22 = nn.Linear(128, 1)
        self.sigmoid2 = nn.Sigmoid()

        # self.linear111 = nn.Linear(784*128, 128)
        # self.bn5 = nn.BatchNorm1d(128)
        # self.relu5 = nn.ReLU()
        # self.linear222 = nn.Linear(128, 1)
        # self.sigmoid3 = nn.Sigmoid()
        # self.linear11 = nn.Linear(784*2048, 128)
        # self.bn4 = nn.BatchNorm1d(128)
        # self.relu4 = nn.ReLU()
        nn.init.kaiming_normal_(self.conv1.weight)
        nn.init.kaiming_normal_(self.conv2.weight)
        # nn.init.kaiming_normal_(self.linear1.weight)
        # nn.init.kaiming_normal_(self.linear2.weight)
        nn.init.kaiming_normal_(self.linear11.weight)
        nn.init.kaiming_normal_(self.linear22.weight)
        # nn.init.kaiming_normal_(self.linear111.weight)
        # nn.init.kaiming_normal_(self.linear222.weight)
        # nn.init.kaiming_normal_(self.linear11.weight)
    def forward(self, x):
       # x11 = x.view(x.shape[0],-1)
        
        x = x.reshape(x.shape[0],1024*2,28,28)
        x1 = self.relu1(self.bn1(self.conv1(x)))
        x2 = self.relu2(self.bn2(self.conv2(x1)))
        # x1 = x1.view(x1.shape[0],-1)
        x2 = x2.view(x2.shape[0],-1)

    #     x1 = self.relu3(self.bn3(self.linear1(x1)))
    #    # x = self.linear2(x)
    #    # x11 = self.relu4(self.bn4(self.linear11(x11)))
    #   #  x = torch.cat([x,x11],dim=0)
    #     x1 =  self.linear2(x1)
    #     x1 = self.sigmoid(x1)
        
        x2 = self.relu4(self.bn4(self.linear11(x2)))
        x2 =  self.linear22(x2)
        x2 = self.sigmoid2(x2)
        return x2

class BaseEncoder(nn.Module): # contrastive的BaseEncoder

    def __init__(self, num_classes=4):
        super(BaseEncoder, self).__init__()
        self.conv1 = nn.Conv2d(1024,256,1)
        self.bn1 = nn.BatchNorm2d(256)  
        self.relu1 = nn.ReLU()
        self.conv2 = nn.Conv2d(256,128,1)
        self.bn2 = nn.BatchNorm2d(128)
        self.relu2 = nn.ReLU()
        self.linear1 = nn.Linear(784*128, 128)
        self.bn3 = nn.BatchNorm1d(128)
        self.relu3 = nn.ReLU()
        nn.init.kaiming_normal_(self.conv1.weight)
        nn.init.kaiming_normal_(self.conv2.weight)
    ...
    def forward(self, x, vars=None):
        if vars is not None:
            self.update_model_parameters(vars)
...
class contrastive(nn.Module):
    """
    Build a contrastive model with: a query encoder, a key encoder, and a queue
    https://arxiv.org/abs/1911.05722
    """

    def __init__(self, encoder, dim=128, K=40, m=0.999, T=0.07):
        """
        dim: feature dimension (default: 128)
        K: queue size; number of negative keys (default: 65536)
        m: contrastive momentum of updating key encoder (default: 0.999)
    ...
    def _dequeue_and_enqueue(self, keys):
        ...
        self.queue = keys.T#[:, ptr : ptr + batch_size] = keys.T
        ptr = (ptr + batch_size) % self.K  # move pointer

        self.queue_ptr[0] = ptr

    def forward(self, im_q, im_k, vars=None):
        """
        Input:
        ...
        """
        if vars is not None:
            self.encoder_q.update_model_parameters(vars)
        # compute query features
        q = self.encoder_q(im_q)  # queries: NxC
        q = nn.functional.normalize(q, dim=1)
        logits = []
        for i in range(im_k.shape[0]):
            k = self.encoder_q(im_k[i])  # keys: NxC   
            k = nn.functional.normalize(k, dim=1)
            self._dequeue_and_enqueue(k)
            logits.append(torch.einsum("nc,ck->nk", [q, self.queue.clone().detach()]))  #  10*802816  802816*20-> 10*20 
        logits = torch.cat(logits,dim=0) # (n*n)*k
        logits /= self.T
        return logits

class GCN(torch.nn.Module): # deprecated
    def __init__(self, encoder ,shot_num, class_num):
        super(GCN, self).__init__()
        self.encoder = encoder
        self.conv1 = GCNConv(128, 256)
        self.conv2 = GCNConv(256, 128)
        self.fc = nn.Linear(128,class_num)
        self.num_nodes = shot_num  
        self.indices = []
        for i in range( self.num_nodes ):
            for j in range(self.num_nodes):
                if i != j:  
                    self.indices.append((i, j))
        self.indices = torch.tensor(self.indices, dtype=torch.long).t().contiguous()
    ...
    def forward(self, query, support):
        ...
        x = self.conv1(x, self.indices)
        x = F.relu(x)
        x = self.conv2(x, self.indices)
        x = F.relu(x)
        x = self.fc(x)
        return x[0:1]
