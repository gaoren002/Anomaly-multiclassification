...
def load(name, pretrained_net_for_classfication):
    if not pretrained_net_for_classfication:
        _BACKBONES[name] = _BACKBONES[name].replace("pretrained=True", "pretrained=False")
