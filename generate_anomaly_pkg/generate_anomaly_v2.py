...
import sys
a = sys.path
from data_loader_for_draem_v2 import MVTecDRAEMTrainDataset
b = sys.path
...
for obj_name in obj_list:
    data_idx = -1
    dataset = MVTecDRAEMTrainDataset(data_path + "/"+obj_name + "/train/good", anomaly_source_path, resize_shape=[256, 256],generate_times=1,shape_max=6)
    ...
    for sample in dataloader:
        ...
        if os.path.exists("./generate_anomaly_pkg/sythesized_anomaly/"+obj_name+"/"+sample['anomaly_class'][0]+"_"+str(sample["shape"][0].item()))==False:
            os.mkdir("./generate_anomaly_pkg/sythesized_anomaly/"+obj_name+"/"+sample['anomaly_class'][0]+"_"+str(sample["shape"][0].item()))
        if os.path.exists("./generate_anomaly_pkg/sythesized_anomaly_mask/"+obj_name+"/"+sample['anomaly_class'][0]+"_"+str(sample["shape"][0].item()))==False:
            os.mkdir("./generate_anomaly_pkg/sythesized_anomaly_mask/"+obj_name+"/"+sample['anomaly_class'][0]+"_"+str(sample["shape"][0].item()))
        if os.path.exists("./generate_anomaly_pkg/sythesized_anomaly_mask_ori/"+obj_name+"/"+sample['anomaly_class'][0]+"_"+str(sample["shape"][0].item()))==False:
            os.mkdir("./generate_anomaly_pkg/sythesized_anomaly_mask_ori/"+obj_name+"/"+sample['anomaly_class'][0]+"_"+str(sample["shape"][0].item()))
        Image.fromarray((sample['augmented_image'].numpy()[0].transpose(1,2,0)*255.0).astype(np.uint8)).save("./generate_anomaly_pkg/sythesized_anomaly/"+obj_name+"/"+
                                                                                                                ...
                                                                                                                "_"+
                                                                                                                str(sample["shape"][0].item())+"/"+str(sample["idx"].numpy()[0])+
        ...
        mymask_name = "./generate_anomaly_pkg/sythesized_anomaly_mask/"+obj_name+"/"+\
                        sample['anomaly_class'][0]+\
                        "_"+str(sample["shape"][0].item())+"/"+str(sample["idx"].numpy()[0])+\
                        "_"+str(sample["anomaly_source_idx"].numpy()[0])+\
                        "_mask.png"
